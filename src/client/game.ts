import { CalcNextX, CreateObj } from "./tools.js";
import { createObstacle, aObstacles, aObstaclesR, CCoins, CPit, CCar, CGlobData } from './classes.js'
...
document.addEventListener('DOMContentLoaded', async function () {//Добавляем обработчик собятия "Loaded..."
    ...
    if (level) {//Проверям есть ли такой параметр
        ...
        const oLevelInfo = locations.find(it => it.id === level);//Находм элемент в массиве у которого АЙДИ совпадает с параметром
        if (oLevelInfo) {
            if (domLevelInfo) {
                domLevelInfo.innerText = oLevelInfo.name;
            }
            speed = oLevelInfo.speed;//easy: 30 medium: 50
